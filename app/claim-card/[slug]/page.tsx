  ...
  if (hasSupabaseServerConfig()) {
    ...
    if (user) {
      const { data: profile } = await supabase
        .from("profiles")
        .select("id, company_id, first_name, last_name, email, team_id, team:teams!profiles_team_id_fkey(name)")
        .eq("id", user.id)
        .maybeSingle<{
          id: string;
      ...
      if (profile?.company_id) {
        const { data: companyProfiles } = await supabase
          .from("profiles")
          .select("id, first_name, last_name, email, team_id, team:teams!profiles_team_id_fkey(name)")
