...
export default async function AdminCompanyDetailPage({ params }: { params: Promise<{ companyId: string }> }) {
  ...
  const [
    ...
    supabase.from("invitations").select("id, email, role, status, expires_at").eq("company_id", companyId).order("created_at", { ascending: false }).limit(8),
        ...
        <article className="panel dashboard-panel">
          <div className="panel-top">
            <div>
              ...
              <p className="section-copy">Open onboarding links for this company.</p>
            </div>
          </div>
