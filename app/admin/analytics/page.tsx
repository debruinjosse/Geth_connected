...
import { BarChart3, Building2, CreditCard, UsersRound } from "lucide-react";
...
export default async function AdminAnalyticsPage() {
      ...
      <DashboardShell role="admin" title="Platform analytics" subtitle="Monitor adoption, volume, and recognition energy across the full ecosystem." user={superAdminUser} actions={<span className="quality-pill">Demo fallback</span>}>
        <section className="dashboard-grid two">
          <article className="panel dashboard-panel">
            ...
            <LineChart color="var(--theme-ink)" points={platformGrowthPoints} labels={["Apr", "May", "Jun", "Jul"]} />
          </article>
          <article className="panel dashboard-panel">
  ...
  return (
    <DashboardShell
      ...
      actions={<span className="quality-pill">Live analytics</span>}
    >
      ...
      <section className="dashboard-grid two">
        <article className="panel dashboard-panel">
          <div className="panel-top"><h2>Monthly recognitions</h2></div>
          {recognitions?.length ? (
            <LineChart color="var(--theme-ink)" points={monthWindows.map((month) => monthlyCounts.get(month.key) ?? 0)} labels={monthWindows.map((month) => month.label)} />
          ) : (
