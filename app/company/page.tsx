...
function getDonutBackground(segments: Array<{ share: number; color: string }>) {
  if (!segments.some((segment) => segment.share > 0)) {
    return "conic-gradient(rgba(42, 23, 61, 0.12) 0% 100%)";
  }
...
function DemoCompanyDashboard() {
  const donut = `conic-gradient(${companyCategoryShare
    .map((segment, index) => {
      const start = companyCategoryShare.slice(0, index).reduce((sum, item) => sum + item.value, 0);
    ...
    <DashboardShell role="company" title="ABC Company" subtitle="Your culture at a glance." user={companyAdmin} actions={<span className="quality-pill">This quarter</span>}>
      ...
      <section className="dashboard-grid two">
        <article className="panel dashboard-panel">
          <div className="panel-top"><h2>Recognition by category</h2></div>
          <div className="donut-layout">
            ...
            <div>
              {companyCategoryShare.map((segment) => (
                <div className="bar-row" key={segment.label}>
                  <span>{segment.label}</span>
                  <div className="bar-track"><span style={{ width: `${segment.value}%`, background: segment.color }} /></div>
      ...
      <section className="dashboard-grid two">
        <article className="panel dashboard-panel">
          <div className="panel-top"><h2>Recognition trend</h2></div>
          <LineChart color="var(--theme-ink)" points={companyTrendThisQuarter} labels={["Apr", "May", "Jun"]} />
          <div style={{ marginTop: 14 }}>
            <LineChart color="var(--theme-gold)" points={companyTrendLastQuarter} labels={["Apr", "May", "Jun"]} />
          </div>
        </article>
        <article className="panel dashboard-panel">
...
export default async function CompanyDashboardPage() {
  ...
  const [{ data: company, error: companyError }, { data: profiles, error: profilesError }, { data: teams, error: teamsError }, { data: recognitions, error: recognitionsError }] =
    await Promise.all([
      ...
      supabase
        ...
        .select("id, team_id, receiver_user_id, created_at, card:card_library(title, category)")
  ...
  const categorySegments = Object.entries(categoryMeta).map(([category, meta]) => {
  ...
  const donut = getDonutBackground(categorySegments.map(({ share, color }) => ({ share, color })));
    ...
    >
            ...
            <div className="donut-layout">
              ...
              <div>
                {categorySegments
                  ...
                  .map((segment) => (
                    <div className="bar-row" key={segment.category}>
                      <span>{segment.label}</span>
                      <div className="bar-track"><span style={{ width: `${segment.roundedShare}%`, background: segment.color }} /></div>
                      <strong>{segment.roundedShare}%</strong>
                    </div>
                  ))}
              </div>
            </div>
      ...
      <section className="dashboard-grid two">
        <article className="panel dashboard-panel">
          ...
          {companyRecognitions.length ? (
            <>
              <LineChart color="var(--theme-ink)" points={trendPoints} labels={trendLabels} />
              <div style={{ marginTop: 14 }}>
                <LineChart color="var(--theme-gold)" points={comparisonPoints} labels={comparisonLabels} />
              </div>
            </>
          ) : (
            <EmptyState
              eyebrow="Trend unavailable"
              title="Recognition trend needs company activity"
