...
/**
 * Core claim/give recognition logic, shared by the web Server Action
 * (`app/actions/claimRecognition.ts`) and the mobile API
 * (`app/api/mobile/v1/recognitions/*`). Callers are responsible for resolving `user` from
 * whatever auth transport they use (cookies for web, a bearer token for mobile) and for passing a
 * `supabase` client that carries that user's identity, so the RLS-scoped queries/inserts below are
 * correctly scoped. Do not duplicate this logic elsewhere — extend it here instead.
 */
...
export type ClaimRecognitionSuccess = { cardTitle: string; mode: "supabase" };

export type GiveRecognitionInput = {
  cardSlug: string;
...
export type GiveRecognitionSuccess = {
  cardTitle: string;
  receiverName: string;
  mode: "supabase";
...
/** Employee self-serve claim of a physical card they received in person. See `claimRecognition` for the full contract. */
export async function claimRecognitionCore(
  supabase: SupabaseClient,
  user: User,
  input: ClaimRecognitionInput
...
/** Employee sends a card digitally to a named colleague in the same company. See `giveRecognition` for the full contract. */
export async function giveRecognitionCore(
  supabase: SupabaseClient,
  user: User,
