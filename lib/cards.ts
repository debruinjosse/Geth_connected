...
export function getLocalizedCardTitle(cardOrTitle: Pick<GethCard, "title"> & Partial<Pick<GethCard, "slug">> | string, locale?: string) {
  ...
  if (normalizedLocale === "en") return slug ? getCanonicalCardBySlugOrNumber(null, slug)?.title ?? title : title;
...
export function mapCardLibraryRowToCard(row: CardLibraryRow): GethCard {
  ...
  return {
    ...
    title: canonicalCard?.title ?? cleanText(row.title),
    category: canonicalCard?.category ?? cleanText(row.category),
    description: canonicalCard?.description ?? cleanText(row.description),
    recognitionSentence: canonicalCard?.recognitionSentence ?? cleanText(row.recognition_sentence),
